  ...
  return (
    <>
      ...
      <AppModal
        ...
        subtitle="Passe Stammdaten, Status und optionale Felder an."
        ...
        footer={
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              {isDeleting ? 'Löschen...' : 'Mitarbeiter löschen'}
            </button>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="submit"
                form={`employee-edit-form-${employee.id}`}
      ...
      >
        <form
          id={`employee-edit-form-${employee.id}`}
          action={handleSubmit}
          className="space-y-5"
        >
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <input
                name="entryDate"
                type="date"
                defaultValue={employee.entry_date ?? ''}
                className="w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
              />
            </div>
          </div>
          ...
          <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/60 px-4 py-3 text-sm text-slate-700">
            <input
              name="isActive"
              type="checkbox"
              defaultChecked={Boolean(employee.is_active)}
